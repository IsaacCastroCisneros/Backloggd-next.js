...
export default async function page({params}:any) 
{
  ...
  const {res:results}=JSON.parse(await get({query:"select * from game where user_id=? and review!=?",data:[user.id,""]})) 
  
  const reviews: Array<review> = await Promise.all<Promise<review>>
  (
    results.map(async (game:gameSlug) => {
